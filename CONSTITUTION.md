...
1. **Simple infrastructure.** Runs on Eviebot (Mac Mini) behind Tailscale Funnel. One process, one machine, `launchd` for persistence. No cloud compute, no containers.
...
- **Deployment target:** Tailscale Funnel on Eviebot Mac Mini
- **Region:** N/A (runs locally on Eviebot)
...
|Eviebot over Lambda              |Already running, proven with Fastmail MCP, simpler debugging, no cold starts          |